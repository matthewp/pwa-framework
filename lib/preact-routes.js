  ...
  async function tryRenderPage(request, reply) {
    ...
    try {
      ...
      try {
        ...
        // TERRIBLE HACK
        let Server = require(path.join(process.cwd(), 'node_modules', '@matthewp', 'pwa-framework', 'Server', 'dist', 'node', 'index.js'));
        let Provider = Server.Settings.Provider;

        let entry = require(path.join(process.cwd(), 'entry-server.js'));
        let { Head } = require(path.join(process.cwd(), 'node_modules', '@matthewp', 'pwa-framework'));

        const render = () => {
          return h(Provider, { value: { start: '/start.js' } },
            h(entry.render, null, 
